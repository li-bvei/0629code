...
2026-08-07（第七轮）撤销 Swift/IBAN 注释 + 新增税込/税抜切换输入（详见 `AI_CONTEXT.md` 2 章节「同日追加：撤销 Swift/IBAN 注释 + 新增税込/税抜切换输入」小节）：

用户反馈 3 点：①請求書去掉上一轮加的"(Swift number/IBAN)"注释（已撤销，「預金種目」拆分行保留）；②怀疑小計/消費税没有验证非課税，统一按课税算；③想加税込/税抜切换，输入时能选金额含不含税。

排查②：分别用 `calculate_voucher_amounts()` 直接测试、以及走完整 `AccountingVoucher` 模型+`build_invoice_pdf()` 真实 PDF 输出两种方式，测了一条 10%税项+一条非課税项混合的单据，两次小計/消費税/合計都算对（非課税金额算进小計但不进消費税）。**没有复现出计算错误**，怀疑是上一轮参考 PDF 测试图里把"印紙代"误选成了 10%（我自己的测试数据疏忽），已报告给用户但还没等到进一步确认，需要具体复现单据才能继续排查。

实现③：用户明确"非課税不需要税込/税抜切换，課税的話一定要加"。`voucher_calculations.py` 新增 `price_type` 字段（`tax_included`默认/`tax_excluded`），非課税行强制忽略该字段；課税行按选择分别计算（税込＝原逻辑倒算，税抜＝输入即税抜金額、税额另外加）。前端「単価」栏下面新增税込/税抜小下拉（垂直堆叠，避免重演上次項目名下拉横向溢出的问题），非課税时隐藏。

验证：`manage.py check`、`npm run build` 均通过；浏览器实测税込¥10,000（10%）→ 小計¥9,091/消費税¥909/合計¥10,000，切税抜同样¥10,000 → 小計¥10,000/消費税¥1,000/合計¥11,000，均与手算一致；完整提交后端返回数据和下载 PDF 上数字一致，测试数据已清理。

状态：

```text
已完成（②非课税计算问题未复现，需要用户提供具体单据才能继续排查）
```

## Previous Task Notes（請求書・領収書样式对照参考模板修正）

2026-08-04（第六轮）請求書・領収書样式对照参考模板修正（详见 `AI_CONTEXT.md` 2 章节「請求書・領収書样式对照参考模板修正」小节）：

用户想重构請求書・領収書样式，提供了真实使用中的 PDF（請求書-白全楽/領収書）和对应旧版 Excel 源文件作参考。用 openpyxl 读取 Excel 结构（列宽/边框/字体）、用真实数据生成当前样式 PDF 做视觉对比后确认：现有实现已经跟参考 PDF 很接近，不是从零重构，只有 5 处差异，逐一确认方案后实施：

1. 敬称写死"御中"→新增 `AccountingVoucher.recipient_honorific` 字段（御中/様/なし下拉，默认御中，安全加法迁移），前端宛先名旁加下拉（用了正确的可收缩 flex 布局，没有重演上次項目名下拉的溢出 bug）。
2. 請求書明細表删除「税区分」列（5列: 適用/単価/数量/金額/備考），前端录入表单的税区分输入框保留（仍用于计算消费税，只是不再打印）。
3. 表头去掉"（税込）"三个字（単価/金額）。
4. 支払い銀行「口座番号」拆成「預金種目」+「口座番号」两行，口座番号后追加"(Swift number / IBAN)"注释。
5. 領収書明细表删除 小計/消費税/合計 三行——但保留同样行数的无边框空白占位行，因为収入印紙章印框的高度位置是靠这几行的高度总和算出来的，直接删空会让印紙框错位。

验证：`manage.py check`、迁移、`npm run build` 均通过；用参考 PDF 里的真实姓名/公司名（白全楽/様、焱途合同会社/御中）生成請求書・領収書 PDF 转图片，逐项跟参考 PDF 核对确认 5 点改动全部生效；浏览器确认敬称下拉不溢出。
...
## Previous Task Notes（請求書・領収書明細入力体验改善）
