...
### 請求書・領収書样式对照参考模板修正（2026-08-04）

用户提出想"重构整体請求書和領収書的样式"，提供了两份参考文件：真实使用中的 PDF（`請求書-白全楽`/`領収書`，视为当前最新、最权威的目标样式）和对应的旧版 Excel 源文件（`請求書.xlsx`/`領収書_.xlsx`，用 openpyxl 读取列宽/合并单元格/字体/边框等结构，仅作为网格参数的参考——Excel 版本内容更旧，比如没有登録番号、没有 小計/消費税/合計 三行拆分，已确认以 PDF 为准）。

排查后发现现有实现其实已经跟参考 PDF 非常接近（这套 `pdf.py` 本来就是照着这个模板家族做的），不是从零重构，实际差异只有 5 处，逐一修正：

- **敬称写死"御中"**：`recipient_with_honorific()`（`backend/apps/accounting/pdf.py`）原来无条件在宛先名后加"御中"，个人客户应该用"様"。新增 `AccountingVoucher.recipient_honorific` 字段（`choices=[御中,様,なし]`，默认"御中"，迁移 `apps/accounting/migrations/0013_accountingvoucher_recipient_honorific.py`，安全加法迁移），`recipient_with_honorific()` 改为读这个字段（`None`/未设置时兜底成"御中"，兼容迁移前的旧数据）。前端 `AccountingVouchersPage.vue` 宛先会社名旁边加一个「敬称」下拉（`.voucher-recipient-control` flex 布局，选择框固定 96px、输入框 `flex:1 1 auto`，两者都加了 `min-width:0`——这次特意用了跟上次"項目名下拉引发溢出"那次事故不一样的、正确的可收缩写法，浏览器验证过不会重演那次的溢出问题）。
- **請求書明細表少了一列「税区分」**：参考模板明细表只有 適用/単価/数量/金額/備考 五列，没有逐行税率百分比列。`build_invoice_pdf()` 里删掉了这一列（连同 `col_widths`/`detail_rows`/`build_invoice_summary_rows` 的 `leading_blank_span` 从 4 改成 3，因为少了一列后合并单元格的跨度要对应缩小，否则汇总行的金额会因为 span 超出总列数而画不出来）。前端明细录入表单里的「税区分」输入框**没有删**——那是用来计算消费税金额的实际数据来源，只是不再打印到 PDF 上（后端仍然拿这个值算 消費税/合計，只是不再单独占一列显示）。
- **表头"（税込）"三个字**：参考模板表头就是"単価"/"金額"，没有"（税込）"。請求書、領収書两处表头文字都去掉了这三个字（金额本身已经是税込金额，字面上的"（税込）"纯粹是冗余）。
- **支払い銀行的口座番号行，参考模板多了"(Swift number / IBAN)"小字**：`parse_bank_info()` 原来把"账户类型+账号"（比如"普通　1667066"）整体塞进一行叫"口座番号"；参考模板实际是拆成「預金種目」「口座番号」两行分别显示。改成把 bank_info 的第二行进一步拆开（第一个词当账户类型，其余当账号），新增「預金種目」这一行，「口座番号」这一行的文字后面追加"　(Swift number / IBAN)"这个固定注释。用项目里已有的两个银行预设文案（大阪信用金庫/GMOあおぞらネット銀行，格式都是"銀行名　支店名\n預金種別　口座番号\n口座名義　..."）验证过能正确拆分。
- **領収書明细表下面多算了「小計/消費税/合計」三行**：参考模板的領収書只在最上面"合計金額"大字框里显示一次税込总额，明细表下面不重复。`build_receipt_pdf()` 不再调用 `build_invoice_summary_rows()` 生成这三行，但**保留了同样数量（3行）的空白、无边框占位行**——因为「収入印紙」章印框的高度和位置是根据这几行的高度总和算出来的（`stamp_top_y = y + sum(receipt_row_heights[receipt_summary_start:])`），如果直接把这几行删空，印紙框会失去参照、位置和高度全部错位。保留空白占位行是最小改动、风险最低的做法，肉眼看不出跟"这几行本来就不存在"有任何区别。

验证：`manage.py check`、`makemigrations`（已生成并本地 `migrate` 过）、`npm run build` 均通过；后端用真实数据（白全楽/様、焱途合同会社/御中）生成請求書和領収書 PDF 转图片，逐项跟参考 PDF 核对过上述 5 点改动全部生效且没有引入新的排版问题；前端浏览器里确认「敬称」下拉正常渲染、不溢出（用 `scrollWidth` 量过）。

### 同日追加：撤销 Swift/IBAN 注释 + 新增税込/税抜切换输入

上一轮加的"(Swift number / IBAN)"注释用户反馈不需要，`build_invoice_pdf()` 里已撤销（「預金種目」单独一行的拆分保留，那是跟参考模板对齐的独立修正，不是这次要撤销的部分）。

用户同时反馈"小計/消費税部分没有验证项目是否非課税，统一按课税算"。排查用 `calculate_voucher_amounts()` 直接测试、以及创建真实 `AccountingVoucher` 走完整 `build_invoice_pdf()` 流程两种方式分别验证过一条 10% 税项 + 一条非課税项混合的单据——两次结果都显示小計/消費税/合計的计算是正确的（非課税那一行的金额会算进小計但不会被加进消費税）。**没有复现出用户描述的计算错误**，怀疑是上一轮我给用户看的参考 PDF 测试图里把"印紙代"这一行随手选成了"10%"（测试数据的疏忽，不是系统 bug），已经把这个疑点报告给用户但还没等到进一步确认——如果之后有具体复现单据，需要重新排查。

用户进一步明确需求："非課税的話不需要保留税込/税抜切換，但課税的話一定要能选输入的钱是税前还是税后"。据此在 `backend/apps/accounting/voucher_calculations.py` 新增 `price_type` 字段（`tax_included` 默认 / `tax_excluded`），新增 `normalize_price_type()`/`line_amounts_from_raw()`：非課税行强制按 `tax_included` 处理（该字段对非課税无意义，`quantity×単価` 直接当最终金额，不产生税额）；課税行按 `price_type` 分两种算法——`tax_included`（沿用原逻辑，从含税总额倒算税抜金額+税額）、`tax_excluded`（新增，税抜金額直接是输入值，税額＝税抜金額×税率，税込総額＝两者相加）。前端 `AccountingVouchersPage.vue` 明細的「単価」栏下面新增一个「税込／税抜」小下拉（`voucher-unit-price-control`/`voucher-price-type-select`，垂直堆叠在输入框下方而不是横向排列，吸取了上次項目名下拉横向挤爆的教训），`tax_category` 选到"非課税"时这个下拉会隐藏。前端 `getLineTotal`/`getTaxExcludedAmount`/`getLineTaxAmount` 预览计算逻辑照抄后端算法保证所见即所得。

验证：`manage.py check`、`npm run build` 均通过；浏览器里现场测试了 税込输入¥10,000（10%）→ 小計¥9,091/消費税¥909/合計¥10,000，切到税抜同样输入¥10,000 → 小計¥10,000/消費税¥1,000/合計¥11,000，两种模式数字均和手算吻合；选非課税后价格类型下拉正确消失；完整提交一条税抜输入的单据，确认后端返回的 `line_items`/`tax_summary` 和下载的 PDF 上"単価/金額/小計/消費税/合計"全部一致，测试数据已清理。
