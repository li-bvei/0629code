      ...
      <el-table v-loading="loading" :data="customers" stripe>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            ...
            <el-button text type="danger" @click="confirmDeleteCustomer(row)">削除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <el-form ref="formRef" :model="customerForm" :rules="rules" label-position="top">
        ...
        <el-form-item label="在留資格" prop="residence_status">
          <el-select v-model="customerForm.residence_status" clearable placeholder="選択してください" class="form-control">
            <el-option
              v-for="status in residenceStatusOptions"
              :key="status"
              :label="status"
